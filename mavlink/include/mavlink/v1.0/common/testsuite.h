...
static void mavlink_test_sonar_interrupt(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
	mavlink_sonar_interrupt_t packet_in = {
		5
    };
	mavlink_sonar_interrupt_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        	packet1.interrupt = packet_in.interrupt;
        
        

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_sonar_interrupt_encode(system_id, component_id, &msg, &packet1);
	mavlink_msg_sonar_interrupt_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_sonar_interrupt_pack(system_id, component_id, &msg , packet1.interrupt );
	mavlink_msg_sonar_interrupt_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_sonar_interrupt_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.interrupt );
	mavlink_msg_sonar_interrupt_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
	...
	mavlink_msg_sonar_interrupt_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_sonar_interrupt_send(MAVLINK_COMM_1 , packet1.interrupt );
	mavlink_msg_sonar_interrupt_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_common(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	...
	mavlink_test_sonar_interrupt(system_id, component_id, last_msg);
